...
- (void)connectionDidFinishLoading:(NSURLConnection *)theConnection
{
	...
	if (!success)
	{
		if ([delegate respondsToSelector:@selector(scheduleFetcher:didFailWithError:)])
			[delegate scheduleFetcher:self didFailWithError:[parser parserError]];
	}
	else
	{
		NSArray *output = [classes copy];
		if ([delegate respondsToSelector:@selector(scheduleFetcher:fetchedClasses:)])
			[delegate scheduleFetcher:self fetchedClasses:output];
	}
	[self cleanup];
}
- (void)connection:(NSURLConnection *)theConnection
...
{
	if ([delegate respondsToSelector:@selector(scheduleFetcher:didFailWithError:)])
		[delegate scheduleFetcher:self didFailWithError:error];
	[self cleanup];
}
