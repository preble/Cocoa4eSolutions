...
- (void)presentImage:(NSImage *)image
{
	...
	posAnim.duration = animationDuration;
	...
	bdsAnim.duration = animationDuration;
...
{
	// Force the text field to apply its value to its binding, animationDuration:
	[[animationDurationTextField window] endEditingFor:animationDurationTextField];
	[CATransaction begin];
	[CATransaction setAnimationDuration:self.animationDuration];
	for (CALayer *layer in [[view layer] sublayers])
	{
		if (layer == [textLayer superlayer]) // the textLayer is contained in a 'textContainer'
			continue;
